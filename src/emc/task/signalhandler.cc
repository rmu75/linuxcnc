...
static void call_gdb(int sig, int start_gdb_in_window)
{
    ...
    if (start_gdb_in_window) {
        snprintf(cmd,
                 ...
                 tmp_gdbrc);
        fprintf(stderr,
                "signal_handler: got signal %d, starting debugger window (pid %d)\n",
                sig,
                getpid());
        ...
        snprintf(cmd,
                 ...
                 getpid());
        fprintf(stderr,
                "signal_handler: got signal %d, generating backtrace in %sbacktrace.%d\n",
                sig,
                dir_prefix,
                getpid());
    }
...
void gdb_backtrace(int sig) { call_gdb(sig, 0); }


void gdb_in_window(int sig) { call_gdb(sig, 1); }
