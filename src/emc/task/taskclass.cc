...
void Task::hal_init_pins(void)
{
    ...
    iocontrol_data.tool_prep_number = 0; /* output, pin that holds the tool number to be prepared,
                                            only valid when tool-prepare=TRUE */
    iocontrol_data.tool_prep_pocket = 0; /* output, pin that holds the pocketno for the tool to be
                                            prepared, only valid when tool-prepare=TRUE */
    iocontrol_data.tool_from_pocket = 0; /* output, always 0 at startup */
    iocontrol_data.tool_prep_index = 0;  /* output, pin that holds the internal index (idx) of the
                                            tool to be prepared, for debug */
    iocontrol_data.tool_change = 0; /* output, notifies a tool-change should happen (emc should be
...
{
    return task_methods
        ->emcToolSetOffset(pocket, toolno, offset, diameter, frontangle, backangle, orientation);
}
...
int emcTaskOnce(const char* filename, EMC_IO_STAT& emcioStatus)
{
    ...
    if (int res = task_methods->iocontrol_hal_init()) { return res; }
    return 0;
}
    ...
    , iocontrol("iocontrol.0")
{
    ...
    if (inifile.Open(ini_filename)) {
        ...
        const char* t;
        if ((t = inifile.Find("TOOL_TABLE", "EMCIO")) != NULL) tooltable_filename = strdup(t);
    }
    ...
    if (!random_toolchanger) {
        ...
        if (tooldata_put(tdata, 0) != IDX_OK) { UNEXPECTED_MSG; }
    }
    if (0 != tooldata_load(tooltable_filename)) { rcs_print_error("can't load tool table.\n"); }
...
static int iniTool(const char* filename)
{
    ...
    if (toolInifile.Open(filename) == false) { return -1; }
    // read the tool change positions
    if (0 != readToolChange(&toolInifile)) { retval = -1; }
    // close the inifile
    toolInifile.Close();
...
void Task::load_tool(int idx)
{
    ...
    if (random_toolchanger) {
        ...
        if (tooldata_get(&tzero, 0) != IDX_OK || tooldata_get(&tpocket, idx) != IDX_OK) {
            UNEXPECTED_MSG;
            return;
        }
        ...
        if (tooldata_put(tzero, idx) != IDX_OK) { UNEXPECTED_MSG; }

        // pocket-->spindle (idx==0)
        tooldata_db_notify(SPINDLE_LOAD, tpocket.toolno, 0, tpocket);
        tpocket.pocketno = 0;
        if (tooldata_put(tpocket, 0) != IDX_OK) { UNEXPECTED_MSG; }

        if (0 != tooldata_save(tooltable_filename)) { emcioStatus.status = RCS_STATUS::ERROR; }
    } else if (idx == 0) {
...
int Task::emcIoInit()  // EMC_TOOL_INIT
{
    ...
    if (0 != iniTool(emc_inifile)) { return -1; }
    return 0;
}
...
int Task::emcToolLoad()  // EMC_TOOL_LOAD_TYPE
{
    ...
    if (random_toolchanger && emcioStatus.tool.pocketPrepped == 0) { return 0; }

    // it's not necessary to load the tool already in the spindle
    CANON_TOOL_TABLE tdata;
    if (tooldata_get(&tdata, emcioStatus.tool.pocketPrepped) != IDX_OK) { UNEXPECTED_MSG; }
                           ...
                           int orientation)  // EMC_TOOL_SET_OFFSET
{
    ...
    tdata.backangle = b;
    tdata.orientation = o;
    if (tooldata_put(tdata, idx) != IDX_OK) { UNEXPECTED_MSG; }
    if (0 != tooldata_save(tooltable_filename)) { emcioStatus.status = RCS_STATUS::ERROR; }
    // TODO
...
int Task::read_tool_inputs(void)
{
    if (iocontrol_data.tool_prepare && iocontrol_data.tool_prepared) {
        emcioStatus.tool.pocketPrepped =
            iocontrol_data.tool_prep_index;  // check if tool has been (idx) prepared
        iocontrol_data.tool_prepare = 0;
        emcioStatus.status =
            RCS_STATUS::DONE;  // we finally finished to do tool-changing, signal task with RCS_DONE
