...
static int no_force_homing =
    0;  // forces the user to home first before allowing MDI and Program run
...
bool jogging_is_active(void) { return emcStatus->motion.jogging_active; }
...
int emcErrorBufferOKtoWrite(int space, const char* caller)
{
    ...
    if (emcErrorBuffer == NULL) return -1;
    if (!emcErrorBuffer->valid()) return -1;
    ...
    if (etime() >= send_errorchan_timout) {
        ...
        // send_errorchan_timout + DEFAULT_EMC_UI_TIMEOUT);
    }
    return 0;
}
...
int emcOperatorError(const char* fmt, ...)
{
    ...
    if (emcErrorBufferOKtoWrite(sizeof(error_msg) * 2, "emcOperatorError")) return -1;
    ...
    vsnprintf(&error_msg.error[strlen(error_msg.error)],
              sizeof(error_msg.error) - strlen(error_msg.error),
              fmt,
              ap);
    va_end(ap);
...
int emcOperatorText(const char* fmt, ...)
{
    ...
    if (emcErrorBufferOKtoWrite(sizeof(text_msg) * 2, "emcOperatorText")) return -1;
...
int emcOperatorDisplay(const char* fmt, ...)
{
    ...
    if (emcErrorBufferOKtoWrite(sizeof(display_msg) * 2, "emcOperatorDisplay")) return -1;
...
static int checkInterpList(NML_INTERP_LIST* il, EMC_STAT* stat)
{
    while (il->len() > 0) {
        ...
        case EMC_TRAJ_LINEAR_MOVE_TYPE: break;

        case EMC_TRAJ_CIRCULAR_MOVE_TYPE: break;
    ...
    interpret_again:
        if (emcTaskPlanIsWait()) {
            ...
            if (readRetval > INTERP_MIN_ERROR || readRetval == INTERP_ENDFILE
                || readRetval == INTERP_EXIT || readRetval == INTERP_EXECUTE_FINISH) {
                ...
                // read
                // through it
                if (programStartLine != 0 && emcTaskPlanLevel() == 0
                ...
                if (emcStatus->task.readLine < programStartLine && emcTaskPlanLevel() == 0) {
                    ...
                    // update the position with our current position, as the other positions are
                    ...
                    if ((emcStatus->task.readLine + 1 == programStartLine)
                        ...
                        // if we happen to execute lines before the current point later (due to
                        // subroutines).
                        programStartLine = 0;
                    }
                }
...
static void mdi_execute_abort(void)
{
    ...
    // XXX: Reset needed?
    if (mdi_execute_wait || mdi_execute_next) emcTaskPlanReset();
    mdi_execute_level = -1;
    mdi_execute_wait = 0;
    mdi_execute_next = 0;
...
static void mdi_execute_hook(void)
{
    ...
    if ((mdi_execute_level < 0) && (mdi_execute_wait == 0) && (mdi_execute_queue.len() > 0)
        && (interp_list.len() == 0) && (emcTaskCommand == NULL)) {
    ...
    if (interp_list.len() == 0 && emcTaskCommand == 0
        && emcStatus->task.execState == EMC_TASK_EXEC::DONE
        && emcStatus->task.interpState != EMC_TASK_INTERP::IDLE && emcStatus->motion.traj.queue == 0
        && emcStatus->io.status == RCS_STATUS::DONE && !mdi_execute_wait && !mdi_execute_next) {
...
void readahead_waiting(void)
{
    ...
    if (interp_list.len() == 0 && emcTaskCommand == 0 && emcStatus->motion.traj.queue == 0
...
static bool allow_while_idle_type()
{
    ...
    case EMC_TRAJ_SET_TELEOP_ENABLE_TYPE: return 1; break;
    }
    return 0;
}
...
static int emcTaskPlan(void)
{
    ...
    case EMC_TASK_STATE::ESTOP_RESET:
        ...
        case EMC_TASK_MODE::MDI:
            ...
            default:
                emcOperatorError(_("command (%s) cannot be executed until the machine is out of "
    ...
    case EMC_TASK_STATE::ON:
        ...
        case EMC_TASK_MODE::MANUAL:  // ON, MANUAL
            ...
            default:
                emcOperatorError(_("can't do that (%s:%d) in manual mode"),
                                 emc_symbol_lookup(type),
                                 (int)type);
                retval = -1;
                break;
...
{
    if (0 == cmd) { return EMC_TASK_EXEC::DONE; }
    ...
    case EMC_TRAJ_SET_FO_ENABLE_TYPE:
    case EMC_TRAJ_SET_FH_ENABLE_TYPE:
    case EMC_TRAJ_SET_SO_ENABLE_TYPE: return EMC_TASK_EXEC::WAITING_FOR_IO; break;
    ...
    case EMC_TOOL_PREPARE_TYPE: return EMC_TASK_EXEC::WAITING_FOR_IO; break;

    case EMC_TOOL_LOAD_TOOL_TABLE_TYPE:
    case EMC_TOOL_SET_OFFSET_TYPE: return EMC_TASK_EXEC::WAITING_FOR_MOTION_AND_IO; break;

    case EMC_TOOL_SET_NUMBER_TYPE: return EMC_TASK_EXEC::WAITING_FOR_IO; break;
    ...
    case EMC_TASK_PLAN_END_TYPE: return EMC_TASK_EXEC::WAITING_FOR_MOTION_AND_IO; break;

    case EMC_TASK_PLAN_INIT_TYPE:
    case EMC_TASK_PLAN_RUN_TYPE:
    case EMC_TASK_PLAN_SYNCH_TYPE:
    case EMC_TASK_PLAN_EXECUTE_TYPE: return EMC_TASK_EXEC::WAITING_FOR_MOTION_AND_IO; break;

    case EMC_TRAJ_DELAY_TYPE: return EMC_TASK_EXEC::WAITING_FOR_MOTION_AND_IO; break;

    case EMC_MOTION_SET_AOUT_TYPE:
        if (((EMC_MOTION_SET_AOUT*)cmd)->now) { return EMC_TASK_EXEC::WAITING_FOR_MOTION; }
        return EMC_TASK_EXEC::DONE;
        break;

    case EMC_MOTION_SET_DOUT_TYPE:
        if (((EMC_MOTION_SET_DOUT*)cmd)->now) { return EMC_TASK_EXEC::WAITING_FOR_MOTION; }
        return EMC_TASK_EXEC::DONE;
        break;

    case EMC_MOTION_ADAPTIVE_TYPE: return EMC_TASK_EXEC::WAITING_FOR_MOTION; break;
...
static int emcTaskIssueCommand(NMLmsg* cmd)
{
    ...
    case EMC_JOG_CONT_TYPE:
        ...
        retval = emcJogCont(jog_cont_msg->joint_or_axis, jog_cont_msg->vel, jog_cont_msg->jjogmode);
        break;

    case EMC_JOG_STOP_TYPE:
        jog_stop_msg = (EMC_JOG_STOP*)cmd;
        retval = emcJogStop(jog_stop_msg->joint_or_axis, jog_stop_msg->jjogmode);
        break;
    ...
    case EMC_JOINT_SET_BACKLASH_TYPE:
        ...
        retval = emcJointSetBacklash(set_backlash_msg->joint, set_backlash_msg->backlash);
        break;
    ...
    case EMC_JOINT_SET_FERROR_TYPE:
        ...
        retval = emcJointSetFerror(set_ferror_msg->joint, set_ferror_msg->ferror);
        break;

    case EMC_JOINT_SET_MIN_FERROR_TYPE:
        set_min_ferror_msg = (EMC_JOINT_SET_MIN_FERROR*)cmd;
        retval = emcJointSetMinFerror(set_min_ferror_msg->joint, set_min_ferror_msg->ferror);
        break;

    case EMC_JOINT_SET_MAX_POSITION_LIMIT_TYPE:
        set_max_limit_msg = (EMC_JOINT_SET_MAX_POSITION_LIMIT*)cmd;
        retval = emcJointSetMaxPositionLimit(set_max_limit_msg->joint, set_max_limit_msg->limit);
        break;

    case EMC_JOINT_SET_MIN_POSITION_LIMIT_TYPE:
        set_min_limit_msg = (EMC_JOINT_SET_MIN_POSITION_LIMIT*)cmd;
        retval = emcJointSetMinPositionLimit(set_min_limit_msg->joint, set_min_limit_msg->limit);
        break;
    ...
    case EMC_TRAJ_SET_FO_ENABLE_TYPE:
        retval = emcTrajSetFOEnable(
            ((EMC_TRAJ_SET_FO_ENABLE*)cmd)->mode);  // feed override enable/disable
        break;

    case EMC_TRAJ_SET_FH_ENABLE_TYPE:
        retval =
            emcTrajSetFHEnable(((EMC_TRAJ_SET_FH_ENABLE*)cmd)->mode);  // feed hold enable/disable
        break;

    case EMC_TRAJ_SET_SO_ENABLE_TYPE:
        retval = emcTrajSetSOEnable(
            ((EMC_TRAJ_SET_SO_ENABLE*)cmd)->mode);  // spindle speed override enable/disable
        break;

    case EMC_TRAJ_SET_VELOCITY_TYPE:
        emcTrajSetVelocityMsg = (EMC_TRAJ_SET_VELOCITY*)cmd;
        retval =
            emcTrajSetVelocity(emcTrajSetVelocityMsg->velocity, emcTrajSetVelocityMsg->ini_maxvel);
        break;
    ...
    case EMC_TRAJ_ABORT_TYPE: retval = emcTrajAbort(); break;
    ...
    case EMC_TRAJ_SET_SPINDLESYNC_TYPE:
        ...
        retval = emcTrajSetSpindleSync(emcTrajSetSpindlesyncMsg->spindle,
    ...
    case EMC_TRAJ_SET_G92_TYPE:
        ...
        retval = 0;
        break;
    case EMC_TRAJ_CLEAR_PROBE_TRIPPED_FLAG_TYPE: retval = emcTrajClearProbeTrippedFlag(); break;
    ...
    case EMC_AUX_INPUT_WAIT_TYPE:
        ...
        if (emcAuxInputWaitMsg->timeout
            == WAIT_MODE_IMMEDIATE) {  // nothing to do, CANON will get the needed value when asked
                                       // by the interp
            ...
            emcAuxInputWaitType =
                emcAuxInputWaitMsg->wait_type;                 // remember what we are waiting for
            emcAuxInputWaitIndex = emcAuxInputWaitMsg->index;  // remember the input to look at
            emcStatus->task.input_timeout =
                2;  // set timeout flag, gets cleared if input changes before timeout happens
            // set the timeout clock to expire at 'now' + delay time
    ...
    case EMC_SPINDLE_SPEED_TYPE:
        ...
        retval = emcSpindleSpeed(spindle_speed_msg->spindle,
                                 spindle_speed_msg->speed,
                                 spindle_speed_msg->factor,
                                 spindle_speed_msg->xoffset);
        break;

    case EMC_SPINDLE_ORIENT_TYPE:
        spindle_orient_msg = (EMC_SPINDLE_ORIENT*)cmd;
        retval = emcSpindleOrient(spindle_orient_msg->spindle,
                                  spindle_orient_msg->orientation,
                                  spindle_orient_msg->mode);
        break;

    case EMC_SPINDLE_ON_TYPE:
        spindle_on_msg = (EMC_SPINDLE_ON*)cmd;
        retval = emcSpindleOn(spindle_on_msg->spindle,
                              spindle_on_msg->speed,
                              spindle_on_msg->factor,
    ...
    case EMC_COOLANT_MIST_ON_TYPE: retval = emcCoolantMistOn(); break;

    case EMC_COOLANT_MIST_OFF_TYPE: retval = emcCoolantMistOff(); break;

    case EMC_COOLANT_FLOOD_ON_TYPE: retval = emcCoolantFloodOn(); break;

    case EMC_COOLANT_FLOOD_OFF_TYPE: retval = emcCoolantFloodOff(); break;
    ...
    case EMC_TOOL_LOAD_TYPE: retval = emcToolLoad(); break;

    case EMC_TOOL_UNLOAD_TYPE: retval = emcToolUnload(); break;
    ...
    case EMC_TASK_PLAN_OPEN_TYPE:
        ...
        if (retval > INTERP_MIN_ERROR) { retval = -1; }
        if (-1 == retval) {
    ...
    case EMC_TASK_PLAN_EXECUTE_TYPE:
        ...
        if (execute_msg->command[0] != 0) {
            ...
            if (emcStatus->task.mode == EMC_TASK_MODE::MDI) {
                if (mdi_execute_level < 0) mdi_execute_level = level;
            }
            ...
            case INTERP_FILE_NOT_OPEN:
                // this caused the error msg on M2 in MDI mode - execRetval == INTERP_EXIT which is
                // would be ok (I think). mah
                retval = -1;
                break;
    ...
    case EMC_TASK_PLAN_END_TYPE: retval = 0; break;
...
static EMC_TASK_EXEC emcTaskCheckPostconditions(NMLmsg* cmd)
{
    ...
    case EMC_OPERATOR_ERROR_TYPE:
    case EMC_OPERATOR_TEXT_TYPE:
    case EMC_OPERATOR_DISPLAY_TYPE: return EMC_TASK_EXEC::DONE; break;

    case EMC_SYSTEM_CMD_TYPE: return EMC_TASK_EXEC::WAITING_FOR_SYSTEM_CMD; break;
    ...
    case EMC_TRAJ_SET_FO_ENABLE_TYPE:
    case EMC_TRAJ_SET_FH_ENABLE_TYPE:
    case EMC_TRAJ_SET_SO_ENABLE_TYPE: return EMC_TASK_EXEC::DONE; break;
    ...
    case EMC_COOLANT_MIST_OFF_TYPE:
    case EMC_COOLANT_FLOOD_ON_TYPE:
    case EMC_COOLANT_FLOOD_OFF_TYPE: return EMC_TASK_EXEC::DONE; break;
    ...
    case EMC_TASK_PLAN_SYNCH_TYPE:
    case EMC_TASK_PLAN_EXECUTE_TYPE:
    case EMC_TASK_PLAN_OPTIONAL_STOP_TYPE: return EMC_TASK_EXEC::DONE; break;
    ...
    case EMC_TRAJ_DELAY_TYPE:
    case EMC_AUX_INPUT_WAIT_TYPE: return EMC_TASK_EXEC::WAITING_FOR_DELAY; break;

    case EMC_MOTION_SET_AOUT_TYPE:
    case EMC_MOTION_SET_DOUT_TYPE:
    case EMC_MOTION_ADAPTIVE_TYPE: return EMC_TASK_EXEC::DONE; break;
...
#define STEPPING_CHECK()                                                                           \
    if (stepping) {                                                                                \
        if (!steppingWait) {                                                                       \
            ...
            if (emcStatus->task.currentLine != steppedLine) { break; }                             \
        }                                                                                          \
    }
...
static int emcTaskExecute(void)
{
    ...
    if (emcSystemCmdPid != 0
        && emcStatus->task.execState != EMC_TASK_EXEC::WAITING_FOR_SYSTEM_CMD) {
        if (emc_debug & EMC_DEBUG_TASK_ISSUE) {
    ...
    case EMC_TASK_EXEC::WAITING_FOR_DELAY:
        ...
        if (emcAuxInputWaitIndex >= 0) {
            ...
            default: emcOperatorError("Unknown Wait Mode");
            }
        }
        break;
    ...
    default:
        // coding error
        if (emc_debug & EMC_DEBUG_TASK_ISSUE) { rcs_print_error("invalid execState"); }
        retval = -1;
        break;
    }
    return retval;
...
#define RETRY_INTERVAL 1.0  // seconds between wait tries for a subsystem
    ...
    good = 0;
    do {
        if (NULL != emcCommandBuffer) { delete emcCommandBuffer; }
    ...
    good = 0;
    do {
        if (NULL != emcStatusBuffer) { delete emcStatusBuffer; }
    ...
    good = 0;
    do {
        if (NULL != emcErrorBuffer) { delete emcErrorBuffer; }
    ...
    // get the timer
    if (!emcTaskNoDelay) { timer = new RCS_TIMER(emc_task_cycle_time, "", ""); }
...
static int iniLoad(const char* filename)
{
    ...
    // open it
    if (inifile.Open(filename) == false) { return -1; }

    if (NULL != (inistring = inifile.Find("JOINTS", "KINS"))) {
        // copy to global
        if (1 != sscanf(inistring, "%i", &joints)) { joints = 0; }
    } else {
    ...
    if (NULL != (inistring = inifile.Find("DEBUG", "EMC"))) {
        // copy to global
        if (1 != sscanf(inistring, "%i", &emc_debug)) { emc_debug = 0; }
    } else {
        // not found, use default
        emc_debug = 0;
    }
    ...
    if (NULL != (inistring = inifile.Find("INTERP_MAX_LEN", "TASK"))) {
        if (1 == sscanf(inistring, "%d", &emc_task_interp_max_len)) {
            if (emc_task_interp_max_len <= 0) { emc_task_interp_max_len = saveInt; }
        } else {
    ...
    if (NULL != (inistring = inifile.Find("CYCLE_TIME", "TASK"))) {
        if (1 == sscanf(inistring, "%lf", &emc_task_cycle_time)) {
            ...
            if (emc_task_cycle_time <= 0.0) { emcTaskNoDelay = 1; }
        } else {
            // found, but invalid
    ...
    if (NULL != (inistring = inifile.Find("IO_ERROR", "TASK"))) { io_error = strdup(inistring); }
...
int main(int argc, char* argv[])
{
    ...
    int latency_excursion_factor =
        10;  // if latency is worse than (factor * expected), it's an excursion
    ...
    while (!done) {
        ...
        if ((emcStatus->io.status == RCS_STATUS::ERROR) && emcStatus->io.fault) {
        ...
        if (emcStatus->motion.status == RCS_STATUS::ERROR && emcStatus->motion.on_soft_limit) {
            ...
            if (emcStatus->io.status == RCS_STATUS::ERROR) {
                ...
                if (emcStatus->io.reason < 0) { emcOperatorError(io_error, emcStatus->io.reason); }
            }
        ...
        if (taskPlanError || taskExecuteError || emcStatus->task.execState == EMC_TASK_EXEC::ERROR
            ...
            emcStatus->task.status = RCS_STATUS::ERROR;
                   ...
                   && emcStatus->io.status == RCS_STATUS::DONE && mdi_execute_queue.len() == 0
                   && interp_list.len() == 0 && emcTaskCommand == 0
                   && emcStatus->task.interpState == EMC_TASK_INTERP::IDLE) {
    ...
    rcs_print("task: %u cycles, min=%.6f, max=%.6f, avg=%.6f, %u latency excursions (> %dx "
              "expected cycle time of %.6fs)\n",
              emcStatus->task.heartbeat,
              minTime,
              maxTime,
              (emcStatus->task.heartbeat != 0)
                  ? (endTime - first_start_time) / emcStatus->task.heartbeat
                  : -1.0,
              num_latency_warnings,
