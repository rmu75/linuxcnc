...
static void print_interp_error(int retval)
{
    ...
    if (0 != emcStatus) { emcStatus->task.interpreter_errcode = retval; }
        ...
        for (dct = 0; dct < dmax; dct++) {
            ...
            if (ret < sizeof(path) && 0 == stat(path, &buf)) {
                if (buf.st_mode & S_IXUSR) {
                    ...
                    ret = snprintf(user_defined_fmt[dct],
                                   sizeof(user_defined_fmt[dct]),
                                   "%s/M1%%02d",
                                   expanddir);  // update global
                    ...
                    if (emc_debug & EMC_DEBUG_CONFIG) {
                        rcs_print("emcTaskInit: user-defined function %s found, but not "
                                  "executable, so ignoring\n",
                                  path);
                    }
                }
            }
...
int emcTaskSetState(EMC_TASK_STATE state)
{
    ...
    case EMC_TASK_STATE::OFF:
        ...
        for (t = 0; t < emcStatus->motion.traj.spindles; t++)
            emcSpindleAbort(t);
        emcTrajDisable();
...
{
    if (emcStatus->motion.traj.mode == EMC_TRAJ_MODE::FREE) { return EMC_TASK_MODE::MANUAL; }
    if (emcStatus->motion.traj.mode == EMC_TRAJ_MODE::TELEOP) { return EMC_TASK_MODE::MANUAL; }
...
{
    if (emcStatus->io.aux.estop) { return EMC_TASK_STATE::ESTOP; }

    if (!emcStatus->motion.traj.enabled) { return EMC_TASK_STATE::ESTOP_RESET; }
...
int emcTaskPlanInit()
{
    ...
    if (retval > INTERP_MIN_ERROR) {  // I'd think this should be fatal.
        ...
        if (0 != rs274ngc_startup_code[0]) {
            ...
            if (retval > INTERP_MIN_ERROR) { print_interp_error(retval); }
        }
    }

    if (emc_debug & EMC_DEBUG_INTERP) { rcs_print("emcTaskPlanInit() returned %d\n", retval); }
...
int emcTaskPlanSetWait()
{
    ...
    if (emc_debug & EMC_DEBUG_INTERP) { rcs_print("emcTaskPlanSetWait() called\n"); }

    return 0;
}

int emcTaskPlanIsWait() { return waitFlag; }

int emcTaskPlanClearWait()
{
    waitFlag = 0;

    if (emc_debug & EMC_DEBUG_INTERP) { rcs_print("emcTaskPlanClearWait() called\n"); }
...
int emcTaskPlanSynch()
{
    ...
    if (retval == INTERP_ERROR) { emcTaskAbort(); }

    if (emc_debug & EMC_DEBUG_INTERP) { rcs_print("emcTaskPlanSynch() returned %d\n", retval); }
...
int emcTaskPlanRead()
{
    ...
    if (emc_debug & EMC_DEBUG_INTERP) { rcs_print("emcTaskPlanRead() returned %d\n", retval); }
...
int emcTaskPlanExecute(const char* command)
{
    ...
    if (command != 0) {  // Command is 0 if in AUTO mode, non-null if in MDI mode.
        ...
        if ((*command != 0) && (inpos)) { interp.synch(); }
    }
    int retval = interp.execute(command);
    if (retval > INTERP_MIN_ERROR) { print_interp_error(retval); }
    if (command != 0) { FINISH(); }

    if (emc_debug & EMC_DEBUG_INTERP) { rcs_print("emcTaskPlanExecute(0) return %d\n", retval); }
...
int emcTaskPlanLine()
{
    ...
    if (emc_debug & EMC_DEBUG_INTERP) { rcs_print("emcTaskPlanLine() returned %d\n", retval); }
...
int emcTaskPlanLevel()
{
    ...
    if (emc_debug & EMC_DEBUG_INTERP) { rcs_print("emcTaskPlanLevel() returned %d\n", retval); }
...
int emcTaskUpdate(EMC_TASK_STAT* stat)
{
    ...
    if (emcStatus->motion.traj.id > 0) { stat->motionLine = emcStatus->motion.traj.id; }
    ...
    if (emcStatus->task.mode != EMC_TASK_MODE::AUTO || res_state == INTERP_ERROR) {
...
int emcAbortCleanup(int reason, const char* message)
{
    ...
    if (status > INTERP_MIN_ERROR) print_interp_error(status);
    return status;
}
