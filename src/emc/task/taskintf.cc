...
int emcJointSetType(int joint, unsigned char jointType)
{
    ...
    if (emc_debug & EMC_DEBUG_CONFIG) { rcs_print("%s(%d, %d)\n", __FUNCTION__, joint, jointType); }
    return 0;
}
...
int emcJointSetUnits(int joint, double units)
{
    ...
    if (emc_debug & EMC_DEBUG_CONFIG) { rcs_print("%s(%d, %.4f)\n", __FUNCTION__, joint, units); }
    return 0;
}
...
int emcJointSetHomingParams(int joint,
                            double home,
                            double offset,
                            ...
                            int absolute_encoder)
{
    ...
    if (use_index) { emcmotCommand.flags |= HOME_USE_INDEX; }
    if (encoder_does_not_reset) { emcmotCommand.flags |= HOME_INDEX_NO_ENCODER_RESET; }
    if (ignore_limits) { emcmotCommand.flags |= HOME_IGNORE_LIMITS; }
    if (is_shared) { emcmotCommand.flags |= HOME_IS_SHARED; }
    if (locking_indexer) { emcmotCommand.flags |= HOME_UNLOCK_FIRST; }
    if (absolute_encoder) {
        switch (absolute_encoder) {
        case 0: break;
        case 1:
            emcmotCommand.flags |= HOME_ABSOLUTE_ENCODER;
...
int emcAxisSetMinPositionLimit(int axis, double limit)
{
    ...
    if (axis < 0 || axis >= EMCMOT_MAX_AXIS || !(TrajConfig.AxisMask & (1 << axis))) { return 0; }
...
static int JointOrTrajInited(void)
{
    ...
    for (joint = 0; joint < EMCMOT_MAX_JOINTS; joint++) {
        if (JointConfig[joint].Inited) { return 1; }
    }
    for (spindle = 0; spindle < EMCMOT_MAX_SPINDLES; spindle++) {
        if (SpindleConfig[spindle].Inited) { return 1; }
    }
    if (TrajConfig.Inited) { return 1; }
    return 0;
}
...
int emcJointInit(int joint)
{
    ...
    if (!JointOrTrajInited()) {
        ...
        if (0 != usrmotInit("emc2_task")) { return -1; }
    }
    JointConfig[joint].Inited = 1;
    if (0 != iniJoint(joint, emc_inifile)) { retval = -1; }
    return retval;
}
...
int emcAxisInit(int axis)
{
    ...
    if (0 != iniAxis(axis, emc_inifile)) { retval = -1; }
    return retval;
}
...
int emcSpindleInit(int spindle)
{
    ...
    if (spindle < 0 || spindle >= EMCMOT_MAX_SPINDLES) { return 0; }
    // init emcmot interface
    if (!JointOrTrajInited()) {
        usrmotIniLoad(emc_inifile);
        if (0 != usrmotInit("emc2_task")) { return -1; }
    ...
    if (0 != iniSpindle(spindle, emc_inifile)) { retval = -1; }
    return retval;
}
...
int emcJointOverrideLimits(int joint)
{
    ...
    if (joint >= EMCMOT_MAX_JOINTS) { return 0; }
...
int emcJointUpdate(EMC_JOINT_STAT stat[], int numJoints)
{
    ...
    if (numJoints <= 0 || numJoints > EMCMOT_MAX_JOINTS) { return -1; }
    ...
    for (joint_num = 0; joint_num < numJoints; joint_num++) {
        ...
        stat[joint_num].minHardLimit = (joint->flag & EMCMOT_JOINT_MIN_HARD_LIMIT_BIT ? 1 : 0);
        stat[joint_num].maxHardLimit = (joint->flag & EMCMOT_JOINT_MAX_HARD_LIMIT_BIT ? 1 : 0);
...
{
    if (linearUnits <= 0.0 || angularUnits <= 0.0) { return -1; }
...
int emcTrajSetMaxAcceleration(double acc)
{
    ...
    if (emc_debug & EMC_DEBUG_CONFIG) { rcs_print("%s(%.4g)\n", __FUNCTION__, acc); }
    return 0;
}
...
int emcTrajSetHome(EmcPose home)
{
    ...
    if (std::isnan(home.tran.x) || std::isnan(home.tran.y) || std::isnan(home.tran.z)
        || std::isnan(home.a) || std::isnan(home.b) || std::isnan(home.c) || std::isnan(home.u)
        || std::isnan(home.v) || std::isnan(home.w)) {
...
int emcTrajSetMotionId(int id)
{
    ...
    if (EMC_DEBUG_MOTION_TIME & emc_debug) {
        if (id != TrajConfig.MotionId) { rcs_print("Outgoing motion id is %d.\n", id); }
    }
...
int emcTrajInit()
{
    ...
    if (0 != iniTraj(emc_inifile)) { retval = -1; }
    return retval;
}
...
double emcTrajGetLinearUnits() { return TrajConfig.LinearUnits; }

double emcTrajGetAngularUnits() { return TrajConfig.AngularUnits; }
                      ...
                      int indexer_jnum)
{
    ...
    if (std::isnan(end.tran.x) || std::isnan(end.tran.y) || std::isnan(end.tran.z)
        || std::isnan(end.a) || std::isnan(end.b) || std::isnan(end.c) || std::isnan(end.u)
        || std::isnan(end.v) || std::isnan(end.w)) {
                        ...
                        double acc)
{
    ...
    if (std::isnan(end.tran.x) || std::isnan(end.tran.y) || std::isnan(end.tran.z)
        || std::isnan(end.a) || std::isnan(end.b) || std::isnan(end.c) || std::isnan(end.u)
        || std::isnan(end.v) || std::isnan(end.w) || std::isnan(center.x) || std::isnan(center.y)
        || std::isnan(center.z) || std::isnan(normal.x) || std::isnan(normal.y)
        || std::isnan(normal.z)) {
                 ...
                 unsigned char probe_type)
{
    ...
    if (std::isnan(pos.tran.x) || std::isnan(pos.tran.y) || std::isnan(pos.tran.z)
        || std::isnan(pos.a) || std::isnan(pos.b) || std::isnan(pos.c) || std::isnan(pos.u)
        || std::isnan(pos.v) || std::isnan(pos.w)) {
...
int emcTrajUpdate(EMC_TRAJ_STAT* stat)
{
    ...
    stat->mode = emcmotStatus.motionFlag & EMCMOT_MOTION_TELEOP_BIT
        ? EMC_TRAJ_MODE::TELEOP
        : (emcmotStatus.motionFlag & EMCMOT_MOTION_COORD_BIT ? EMC_TRAJ_MODE::COORD
                                                             : EMC_TRAJ_MODE::FREE);
...
int emcSpindleSetParams(int spindle,
                        double max_pos,
                        double min_pos,
                        double max_neg,
                        double min_neg,
...
int emcSpindleAbort(int spindle) { return emcSpindleOff(spindle); }
...
int emcMotionUpdate(EMC_MOTION_STAT* stat)
{
    ...
    if (0 != usrmotReadEmcmotStatus(&emcmotStatus)) { return -1; }
    new_config = 0;
    if (emcmotStatus.config_num != emcmotConfig.config_num) {
        if (0 != usrmotReadEmcmotConfig(&emcmotConfig)) { return -1; }
        new_config = 1;
    }

    if (get_emcmot_internal_info) {
        if (0 != usrmotReadEmcmotInternal(&emcmotInternal)) { return -1; }
    }
...
int emcGetExternalOffsetApplied(void) { return emcmotStatus.external_offsets_applied; }

EmcPose emcGetExternalOffsets(void) { return emcmotStatus.eoffset_pose; }
