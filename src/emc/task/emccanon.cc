...
#define TO_PROG_LEN(mm)                                                                            \
    ((mm)                                                                                          \
     / (canon.lengthUnits == CANON_UNITS_INCHES   ? 25.4                                           \
 ...
 */
static void to_rotated(PM_CARTESIAN& vec) { rotate(vec.x, vec.y, canon.xy_rotation); }
#if 0
...
static void rotate_and_offset_xyz(PM_CARTESIAN& xyz)
{
    ...
    xyz += PM_CARTESIAN(canon.toolOffset.tran.x, canon.toolOffset.tran.y, canon.toolOffset.tran.z);
}
...
static int axis_valid(int n) { return emcStatus->motion.traj.axis_mask & (1 << n); }
...
static double toExtVel(double vel)
{
    if (canon.cartesian_move && !canon.angular_move) {
        ...
        return TO_EXT_LEN(vel);
    } else {  // seems this case was forgotten, neither linear, neither angular move (we are only
              // sending vel)
        return TO_EXT_LEN(vel);
    }
}
...
static void send_g5x_msg(int index)
{
    ...
    for (int s = 0; s < emcStatus->motion.traj.spindles; s++) {
        if (canon.spindle[s].css_maximum) { SET_SPINDLE_SPEED(s, canon.spindle[s].speed); }
    }
 ...
 */
static double getMinLinearDisplacement() { return FROM_EXT_LEN(CART_FUZZ); }

/**
 * Equivalent of getMinLinearDisplacement for rotary axes.
 */
static double getMinAngularDisplacement() { return FROM_EXT_ANG(CART_FUZZ); }
                                         ...
                                         double w)
{
    ...
    if (canon.cartesian_move && !canon.angular_move) {
        ...
        if (out.tmax > 0.0) { out.acc = out.dtot / out.tmax; }
    }
    // Pure angular move:
    else if (!canon.cartesian_move && canon.angular_move) {
    ...
    // Combination angular and linear move:
        ...
        if (debug_velacc)
            printf("getStraightAcceleration t^2 tx %g ty %g tz %g ta %g tb %g tc %g tu %g tv %g tw "
                   "%g\n",
                   tx,
                   ty,
                   tz,
...
static AccelData getStraightAcceleration(CANON_POSITION pos)
{
    ...
    return getStraightAcceleration(pos.x, pos.y, pos.z, pos.a, pos.b, pos.c, pos.u, pos.v, pos.w);
}
...
static VelData getStraightVelocity(CANON_POSITION pos)
{
    ...
    return getStraightVelocity(pos.x, pos.y, pos.z, pos.a, pos.b, pos.c, pos.u, pos.v, pos.w);
}
...
static void drop_segments(void) { chained_points.clear(); }
...
static void flush_segments(void)
{
    ...
    if (canon.cartesian_move && !canon.angular_move) {
        if (vel > canon.linearFeedRate) { vel = canon.linearFeedRate; }
    } else if (!canon.cartesian_move && canon.angular_move) {
        if (vel > canon.angularFeedRate) { vel = canon.angularFeedRate; }
    } else if (canon.cartesian_move && canon.angular_move) {
        if (vel > canon.linearFeedRate) { vel = canon.linearFeedRate; }
    }
...
static bool
linkable(double x, double y, double z, double a, double b, double c, double u, double v, double w)
{
    struct pt& pos = chained_points.back();
    if (canon.motionMode != CANON_CONTINUOUS || canon.naivecamTolerance == 0) return false;
    ...
    for (std::vector<struct pt>::iterator it = chained_points.begin(); it != chained_points.end();
         it++) {
        PM_CARTESIAN M(x - canon.endPoint.x, y - canon.endPoint.y, z - canon.endPoint.z),
            B(canon.endPoint.x, canon.endPoint.y, canon.endPoint.z), P(it->x, it->y, it->z);
...
{
    bool changed_abc =
        (a != canon.endPoint.a) || (b != canon.endPoint.b) || (c != canon.endPoint.c);

    bool changed_uvw =
        (u != canon.endPoint.u) || (v != canon.endPoint.v) || (w != canon.endPoint.w);

    if (!chained_points.empty() && !linkable(x, y, z, a, b, c, u, v, w)) { flush_segments(); }
    pt pos = {x, y, z, a, b, c, u, v, w, line_number, tag};
    chained_points.push_back(pos);
    if (changed_abc || changed_uvw) { flush_segments(); }
}

void FINISH() { flush_segments(); }

void ON_RESET() { drop_segments(); }
                       ...
                       double w)
{
    ...
    if (canon.feed_mode) STOP_SPEED_FEED_SYNCH();
    ...
    if (old_feed_mode) START_SPEED_FEED_SYNCH(canon.spindle_num, canon.linearFeedRate, 1);
...
void SET_MOTION_CONTROL_MODE(CANON_MOTION_MODE mode, double tolerance)
{
    ...
    case CANON_CONTINUOUS:
        ...
        break;
    case CANON_EXACT_PATH: setTermCondMsg->cond = EMC_TRAJ_TERM_COND_EXACT; break;
                 ...
                 double r = 1.0)
{
    ...
    if (beta1 == cc || beta2 == cc
        || beta > (1 / CART_FUZZ)) {  // "beta > 1e8" added by jjf (without this condition
                                      // "Ruota_dentata #101.ngc" is not working! beta=1e14)
        CANON_POSITION p = unoffset_and_unrotate_pos(canon.endPoint);
        to_prog(p);
        ...
        double p1x = p0x + alpha * tsx, p1y = p0y + alpha * tsy, p3x = p4x - beta * tex,
               p3y = p4y - beta * tey, p2x = (p1x * beta + p3x * alpha) / ab,
                   ...
                   CANON_PLANE plane)
{
    ...
    for (unsigned int i = 1; i <= div; i++) {
        ...
        P1T = nurbs_G5_tangent(u, nurbs_order, nurbs_control_points, knot_vector);
        biarc(lineno,
              P0.NURBS_X,
              P0.NURBS_Y,
              P0T.NURBS_X,
                                        ...
                                        double feedrate)
{
    ...
    std::vector<double> lenght_vector =
        nurbs_lenght_vector_creator(k, nurbs_control_points, knot_vector, span_knot_vector);
    std::vector<double> Du_span_knot_vector =
        nurbs_Du_span_knot_vector_creator(k, nurbs_control_points, knot_vector, span_knot_vector);
    std::vector<double> nurbs_costant =
        nurbs_costant_crator(span_knot_vector, lenght_vector, Du_span_knot_vector);
    ...
    alf1 = (int)alf;
    if (alf1 == 0) { alf1 = 1; }
    if (feedrate >= 1500) { feedrate = feedrate / 10; }
    dl = alf1 * (feedrate * T / 60);
    ltot = nurbs_lenght_tot((lenght_vector.size() - 1), span_knot_vector, lenght_vector);
    ...
    if (canon.activePlane == CANON_PLANE::XY) {
        STRAIGHT_FEED(lineno, P1.NURBS_X, P1.NURBS_Y, p.z, p.a, p.b, p.c, p.u, p.v, p.w);
    }
    if (canon.activePlane == CANON_PLANE::YZ) {
        STRAIGHT_FEED(lineno, p.x, P1.NURBS_X, P1.NURBS_Y, p.a, p.b, p.c, p.u, p.v, p.w);
    }
    if (canon.activePlane == CANON_PLANE::XZ) {
        STRAIGHT_FEED(lineno, P1.NURBS_Y, p.y, P1.NURBS_X, p.a, p.b, p.c, p.u, p.v, p.w);
    }

    double a = 0, du = 0;
    if (k >= 3) {
        for (double l_ = dl; l_ <= ltot - dl; l_ = l_ + dl) {
            u_l = nurbs_uj_l(l_, span_knot_vector, lenght_vector, nurbs_costant);
            du = abs(u_l - a);

            if (u_l >= a) {  // fa un controllo per rimediare a eventuali  errori groddolani di
                             // approssimazione
                P1x = nurbs_G6_point_x(u_l, k, nurbs_control_points, knot_vector);
                if (canon.activePlane == CANON_PLANE::XY) {
        ...
        P1 = nurbs_G6_pointx(knot_vector[n + k], k, nurbs_control_points, knot_vector, A6);
        if (canon.activePlane == CANON_PLANE::XY) {
            STRAIGHT_FEED(lineno, P1.NURBS_X, P1.NURBS_Y, p.z, p.a, p.b, p.c, p.u, p.v, p.w);
        }
        if (canon.activePlane == CANON_PLANE::YZ) {
        ...
        for (double l_ = dl; l_ <= ltot - dl; l_ = l_ + dl) {
            ...
            if (u_l >= a) {  // fa un controllo per rimediare a eventuali  errori grossolani di
                             // approssimazione
                A6 = nurbs_G6_Nmix_creator(u_l, k, n + 1, knot_vector);
                P1 = nurbs_G6_pointx(u_l, k, nurbs_control_points, knot_vector, A6);
                if (canon.activePlane == CANON_PLANE::XY) {
                    STRAIGHT_FEED(lineno,
                                  P1.NURBS_X,
                                  P1.NURBS_Y,
                                          ...
                                          double feedrate)
{
    ...
    double a = 0, du = 0;
    if (k >= 3) {  // l'algoritmo di De Boor non funziona per ordine <3 quindi uso la formulazione
                   // ricorsiva, ossia A6
        P0 = nurbs_G6_point_x(knot_vector[0], k, nurbs_control_points, knot_vector);
        P0T = nurbs_G6_tangent_x(knot_vector[0], k, nurbs_control_points, knot_vector);
        for (double l_ = dl; l_ <= ltot - dl; l_ = l_ + dl) {
            u_l = nurbs_uj_l(l_, span_knot_vector, lenght_vector, nurbs_costant);
            du = abs(u_l - a);
            ...
            if (u_l >= a) {  // fa un controllo per rimediare a eventuali  errori grossolani di
                             // approssimazione
                P1 = nurbs_G6_point_x(u_l, k, nurbs_control_points, knot_vector);
                P1T = nurbs_G6_tangent_x(u_l, k, nurbs_control_points, knot_vector);
                biarc(lineno,
                      P0.NURBS_X,
                      P0.NURBS_Y,
                      P0T.NURBS_X,
                ...
                u_l = a + du / 4;  // il precedente valore di u è incrementato della metta del
                                   // precedente du;
        ...
        for (double l_ = dl; l_ <= ltot - dl; l_ = l_ + dl) {
            ...
            if (u_l >= a) {
                ...
                a = u_l;
            } else {  // fa un controllo per rimediare a eventuali  errori grossolani di
                      // approssimazione
                u_l = a + du / 4;
                A6 = nurbs_G6_Nmix_creator(u_l, k, n + 1, knot_vector);
                P1 = nurbs_G6_pointx(u_l, k, nurbs_control_points, knot_vector, A6);
                       ...
                       int Q_option)
{
    ...
    std::vector<double> control_point_nurbs1_new1 =
        nurbs_G6_new_control_point_nurbs1(ut, nurbs_order, nurbs_control_points, knot_vector);

    NURBS_CONTROL_POINT C_P1;
    for (long unsigned int i = 0; i < control_point_nurbs1_new1.size(); i = i + 3) {
    ...
    // __FILE__,__LINE__); Secondo segmento NURBS
    //  si passa il valore u in corrispondenza del quale suddividere la curva
    //  der u-Wert wird übergeben, bei dem die Kurve geteilt werden soll
    std::vector<double> control_point_nurbs2_new2 =
        nurbs_G6_new_control_point_nurbs2(ut, nurbs_order, nurbs_control_points, knot_vector);

    std::vector<NURBS_CONTROL_POINT> nurbs_control_points2;
    NURBS_CONTROL_POINT C_P2;
    for (long unsigned int i = 0; i < control_point_nurbs2_new2.size(); i = i + 3) {
    ...
    for (int j_segment = 0; j_segment <= 1; ++j_segment) {
        ...
        if (j_segment == 0) {
            for (unsigned j = 0; j < nurbs_control_points1.size(); ++j) {
                ...
                CPU.NURBS_R = nurbs_control_points1[j]
            ...
            knot_vector =
                nurbs_G6_knot_vector_new_creator_sgment(nurbs_order, nurbs_control_points);
        } else {
            ...
            // la porzione di codice aggiunge zeri, l'obiettivo è di far aumentare di k la
            // dimenzione, questo in modo da farne coincidere le dimenzione rispetto a quando la
            // suddivisione non avviene realizzata. Der Teil des Codes fügt Nullen hinzu, das Ziel
            // ist es, die Dimension um k zu erhöhen, um die Dimensionen in Bezug darauf
            // zusammenfallen zu lassen, wenn die Unterteilung nicht stattfindet.
            for (unsigned j = 0; j < nurbs_order; ++j) {
                nurbs_control_points.push_back(CPU);
            }
            knot_vector =
                ...
                if (Q_option == 2) {
                    ...
                    if (Q_option == 1) {
                        ...
                        if (feedrate <= 300) {  // velocità di avanzamento 300mm/min //METODO MISTO
                            // printf("G6.2_L_mist=interpolazione NURBS tramite movimento lineare (è
                            // il controllo a decidere di applicare tale metodo in funzione del
                            // valore ds)\n"); printf("G6.2_L_mist=NURBS interpolation using linear
                            // movement (it is the control that decides to apply this method
                            // according to the ds value)\n");
                            NURBS_FEED_G6_2_WITH_LINEAR_MOTION(lineno,
                                                               nurbs_control_points,
                                                               knot_vector,
                                                               nurbs_order,
                            ...
                            // printf("G6.2_B_mist=interpolazione NURBS tramite movimento circolare
                            // (è il controllo a decidere di applicare tale metodo in funzione del
                            // valore ds)\n");
                            NURBS_FEED_G6_2_WITH_CIRCULAR_MOTION(lineno,
                                                                 nurbs_control_points,
                                                                 knot_vector,
...
//-----------------------------------------------------------------------------------------------------------------------------------------
// Non viene realizzata alcuna suddivisione in quanto il numero dei punti di controllo è <2*k =due
// volte l'ordine No subdivision is made as the number of control points is <3*k = twice the order
        ...
        if (Q_option == 2) {
            ...
            if (Q_option == 1) {
                ...
                if (feedrate <= 300) {  // velocità di avanzamento 300mm/min
                    // printf("G6.2_L_mist=interpolazione NURBS tramite movimento lineare (è il
                    // controllo a decidere di applicare tale metodo in funzione del valore ds)\n");
                    // printf("G6.2_L_mist=NURBS interpolation using linear movement (it is the
                    // control that decides to apply this method according to the ds value)\n");
                    NURBS_FEED_G6_2_WITH_LINEAR_MOTION(lineno,
                                                       nurbs_control_points,
                                                       knot_vector,
                                                       nurbs_order,
                    ...
                    // printf("G6.2_B_mist=interpolazione NURBS tramite movimento circolare (è il
                    // controllo a decidere di applicare tale metodo in funzione del valore ds)\n");
                    // printf("G6.2_B_mist=NURBS interpolation via circular movement (it is the
                    // control that decides to apply this method according to the ds value)\n");
                    NURBS_FEED_G6_2_WITH_CIRCULAR_MOTION(lineno,
                                                         nurbs_control_points,
                                                         knot_vector,
...
{  // (L_option: NICU, NICL, NICC see publication from Lo Valvo and Drago)
    ...
    if (n > (int)(3 * nurbs_order)) {
        ...
        // Verification operation which eventually activates the subdivision of the NURBS into
        // segments

        // printf("n>3*k, è operata una suddivisione della curva NURBS;
        // n=%d\n",nurbs_control_points.size() - 1-k); printf("n>3*k, a subdivision of the NURBS
        // curve is made;  n=%ld (F: %s L: %d)\n",nurbs_control_points.size() - 1 - k, __FILE__,
        // __LINE__);
        NURBS_FEED_DIVIDE(lineno,
                          nurbs_control_points,
                          knot_vector,
              ...
              double w)
{
    ...
    // Rearrange the X Y Z coordinates in the correct order based on the active plane (XY, YZ, or
    // XZ) KLUDGE CANON_PLANE is 1-indexed, hence the subtraction here to make a 0-index value
    int shift_ind = 0;
    ...
    case CANON_PLANE::UV:
    case CANON_PLANE::VW:
    case CANON_PLANE::UW: CANON_ERROR("Can't set plane in UVW axes, assuming XY"); break;
    }
    ...
    // Rotate and offset the new end point to be in the same coordinate system as the current end
    // point
    ...
    */
    ...
    int full_turns = 0;
    if (rotation > 1) { full_turns = rotation - 1; }
    if (rotation < -1) { full_turns = rotation + 1; }
...
auto SPINDLE_SPEED_(int s, int dir, double speed)
{
    ...
    flush_segments();
    if (dir != 0) canon.spindle[s].dir = dir;
    if (speed != 0)
    ...
    if (canon.spindle[s].css_maximum) {
        ...
        emc_spindle_msg->xoffset =
            TO_EXT_LEN(canon.g5xOffset.x + canon.g92Offset.x + canon.toolOffset.tran.x);
    } else {
...
void STOP_SPINDLE_TURNING(int s)
{
    ...
    // Added by atp 6/1/18 not sure this is right. There is a problem that the _second_ S word
    // starts the spindle without M3/M4
...
void CHANGE_TOOL()
{
    ...
    if (have_tool_change_position) {
        ...
        if (vel && acc) tag_and_send(std::move(linearMoveMsg), _tag);
...
void DISABLE_FEED_OVERRIDE() { FEED_OVERRIDE_ENABLE_(0); }

void ENABLE_FEED_OVERRIDE() { FEED_OVERRIDE_ENABLE_(1); }
...
void DISABLE_ADAPTIVE_FEED() { ADAPTIVE_FEED_ENABLE_(0); }

void ENABLE_ADAPTIVE_FEED() { ADAPTIVE_FEED_ENABLE_(1); }
...
void DISABLE_SPEED_OVERRIDE(int spindle) { SPEED_OVERRIDE_(spindle, 0); }

void ENABLE_SPEED_OVERRIDE(int spindle) { SPEED_OVERRIDE_(spindle, 1); }
...
void ENABLE_FEED_HOLD() { FEED_HOLD_(1); }

void DISABLE_FEED_HOLD() { FEED_HOLD_(0); }

template<typename T, bool flush_segments_ = true>
void SIMPLE_COMMAND_()
{
    if (flush_segments_) flush_segments();

    interp_list.append(std::make_unique<T>());
}

void FLOOD_OFF() { SIMPLE_COMMAND_<EMC_COOLANT_FLOOD_OFF>(); }

void FLOOD_ON() { SIMPLE_COMMAND_<EMC_COOLANT_FLOOD_ON>(); }
...
void MIST_OFF() { SIMPLE_COMMAND_<EMC_COOLANT_MIST_OFF>(); }

void MIST_ON() { SIMPLE_COMMAND_<EMC_COOLANT_MIST_ON>(); }
...
void TURN_PROBE_ON() { SIMPLE_COMMAND_<EMC_TRAJ_CLEAR_PROBE_TRIPPED_FLAG, false>(); }
...
void OPTIONAL_PROGRAM_STOP() { SIMPLE_COMMAND_<EMC_TASK_PLAN_OPTIONAL_STOP>(); }
...
double GET_EXTERNAL_TOOL_LENGTH_XOFFSET() { return TO_PROG_LEN(canon.toolOffset.tran.x); }

double GET_EXTERNAL_TOOL_LENGTH_YOFFSET() { return TO_PROG_LEN(canon.toolOffset.tran.y); }

double GET_EXTERNAL_TOOL_LENGTH_ZOFFSET() { return TO_PROG_LEN(canon.toolOffset.tran.z); }

double GET_EXTERNAL_TOOL_LENGTH_AOFFSET() { return TO_PROG_ANG(canon.toolOffset.a); }

double GET_EXTERNAL_TOOL_LENGTH_BOFFSET() { return TO_PROG_ANG(canon.toolOffset.b); }

double GET_EXTERNAL_TOOL_LENGTH_COFFSET() { return TO_PROG_ANG(canon.toolOffset.c); }

double GET_EXTERNAL_TOOL_LENGTH_UOFFSET() { return TO_PROG_LEN(canon.toolOffset.u); }

double GET_EXTERNAL_TOOL_LENGTH_VOFFSET() { return TO_PROG_LEN(canon.toolOffset.v); }

double GET_EXTERNAL_TOOL_LENGTH_WOFFSET() { return TO_PROG_LEN(canon.toolOffset.w); }
...
int GET_EXTERNAL_PROBE_TRIPPED_VALUE() { return emcStatus->motion.traj.probe_tripped; }
...
int GET_EXTERNAL_MIST() { return emcStatus->io.coolant.mist; }

int GET_EXTERNAL_FLOOD() { return emcStatus->io.coolant.flood; }
...
{
    if (emcStatus->motion.spindle[spindle].speed == 0) { return CANON_STOPPED; }

    if (emcStatus->motion.spindle[spindle].speed >= 0.0) { return CANON_CLOCKWISE; }
...
int GET_EXTERNAL_TC_FAULT() { return emcStatus->io.fault; }

int GET_EXTERNAL_TC_REASON() { return emcStatus->io.reason; }

int GET_EXTERNAL_FEED_OVERRIDE_ENABLE() { return emcStatus->motion.traj.feed_override_enabled; }
...
int GET_EXTERNAL_ADAPTIVE_FEED_ENABLE() { return emcStatus->motion.traj.adaptive_feed_enabled; }

int GET_EXTERNAL_FEED_HOLD_ENABLE() { return emcStatus->motion.traj.feed_hold_enabled; }

int GET_EXTERNAL_AXIS_MASK() { return emcStatus->motion.traj.axis_mask; }

int GET_EXTERNAL_OFFSET_APPLIED(void) { return emcGetExternalOffsetApplied(); }

EmcPose GET_EXTERNAL_OFFSETS() { return emcGetExternalOffsets(); }
...
int GET_EXTERNAL_DIGITAL_INPUT(int index, int def)
{
    ...
    if (emcStatus->task.input_timeout == 1) return -1;
...
{
    if (num < 0 || num >= USER_DEFINED_FUNCTION_NUM) { return -1; }
...
int WAIT(int index,      /* index of the motion exported input */
         ...
         double timeout) /* time to wait [in seconds], if the input didn't change the value -1 is
                            returned */
{
