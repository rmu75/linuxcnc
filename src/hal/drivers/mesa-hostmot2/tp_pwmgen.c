    ...
    // FIXME: r hides the r in enclosing function, and it returns the wrong thing
    {
        ...
        *hm2->tp_pwmgen.hal->param.pwm_frequency = 20000;
	    ...
	    rtapi_snprintf(name, sizeof(name), "%s.3pwmgen.%02d.fault", hm2->llio->name, i);
            ...
            r = hal_pin_float_new(name, HAL_OUT, &(hm2->tp_pwmgen.instance[i].hal.param.scale), hm2->llio->comp_id);
            if (r < 0) {
	    ...
	    rtapi_snprintf(name, sizeof(name), "%s.3pwmgen.%02d.deadtime", hm2->llio->name, i);
            r = hal_pin_float_new(name, HAL_OUT, &(hm2->tp_pwmgen.instance[i].hal.param.deadzone), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail2;
            }
	    ...
	    rtapi_snprintf(name, sizeof(name), "%s.3pwmgen.%02d.fault-invert", hm2->llio->name, i);
            r = hal_pin_bit_new(name, HAL_OUT, &(hm2->tp_pwmgen.instance[i].hal.param.faultpolarity), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail2;
            }
	    ...
	    rtapi_snprintf(name, sizeof(name), "%s.3pwmgen.%02d.sample-time", hm2->llio->name, i);
            r = hal_pin_float_new(name, HAL_OUT, &(hm2->tp_pwmgen.instance[i].hal.param.sampletime), hm2->llio->comp_id);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail2;
            }
