/* Autogenerated by /home/rene/dev/linuxcnc-cxx/bin/halcompile on Wed Oct  4 13:59:12 2023 -- do not edit */
#include "rtapi.h"
#ifdef RTAPI
#include "rtapi_app.h"
#endif
...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:foo:Compute the derivative of the input function");
...
RTAPI_MP_INT(count, "number of foo");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of foo");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("foo");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "foo.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
